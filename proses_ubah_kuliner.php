...
if(isset($_POST['ubah_foto'])){ // Jika user menceklis checkbox yang ada di form ubah, lakukan :
  ...
  if(move_uploaded_file($tmp, $path)){ // Cek apakah gambar berhasil diupload atau tidak
    ...
    $query = "UPDATE wisatakuliner SET  nama_kuliner='".$nama_kuliner."', deskripsi='".$deskripsi."', alamat='".$alamat."', notelp_pengelola='".$notelp_pengelola."', maps='".$maps."', foto='".$fotobaru."' WHERE id_kuliner='".$id_kuliner."'";
    $sql = mysqli_query($connect, $query); // Eksekusi/ Jalankan query dari variabel $query
    if($sql){ // Cek jika proses simpan ke database sukses atau tidak
      ...
      echo "<br><a href='form_ubah_kuliner.php'>Kembali Ke Form</a>";
    }
  }else{
  ...
  // Proses ubah data ke Database
  $query = "UPDATE wisatabudaya SET nama_kuliner='".$nama_kuliner."', deskripsi='".$deskripsi."', alamat='".$alamat."', notelp_pengelola='".$notelp_pengelola."', maps='".$maps."', foto='".$fotobaru."' WHERE id_kuliner='".$id_kuliner."'";
  $sql = mysqli_query($connect, $query); // Eksekusi/ Jalankan query dari variabel $query
  if($sql){ // Cek jika proses simpan ke database sukses atau tidak
    // Jika Sukses, Lakukan :
    header("location: wisatakuliner.php"); // Redirect ke halaman galeri.php
